...
(function ($) {
	$.fn.SinaEmotion = function (target) {
		...
		$(this).click(function (event) {
			...
			$(target).parents(".content").append('<div id="emotions"></div>');
			$('#emotions').css({
				bottom: eTop,
				left: eLeft
			});
			...
			$('#emotions').html('<div class="cate-page hide"><a href="javascript:void(0);" id="prev">«</a><a href="javascript:void(0);" id="next">»</a></div><div class="categorys"></div><div class="container"></div>');
			...
			showCategorys();
			showEmotions();
			$('#emotions .container a').eq(0).focus();
		});
		$('body').click(function () {
			$('#emotions').remove();
		});
		...
		function showCategorys() {
			...
			//            }
			$('#emotions .categorys').append($('<a href="javascript:void(0);">' + categorys[0] + '</a>'));
		...
		function showEmotions() {
			...
			for (var i = page * 70; i < (page + 1) * 70 && i < emotions[category].length; ++i) {
			...
			$('#emotions .container img').click(function () {
				target.insertText($(this).parent().attr('title'));
				$('#emotions').remove();
				setTimeout(function () {
					$(document).scrollTop(99999);
				}, 500);
			});
			//            for (var i = 1; i < emotions[category].length / 70 + 1; ++i) {
