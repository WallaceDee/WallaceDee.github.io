...
openSafeKeyBoard: function (len, title, success_callback, fail_callback) {
    ...
    exec_asyn("App", "openSafeKeyBoard", JSON.stringify(_json), success_callback, fail_callback);
},
...
execWhenReturnAndAppear: function (msg) {
    ...
    exec_asyn("App", "execWhenReturnAndAppear", JSON.stringify(msgJson));
},
...
getClientVersion: function () {
    var version = exec_syn("App", "getClientVersionName", null);
    return version;
},
    
getCommonRequestParams: function () {
    return exec_syn("App", "getCommonRequestParams", null);
},
    /**
     * 关闭按钮触发事件
     * @param orderNo
     */
closeH5: function (url,orderNo,keep,userCode,token,prodId) {
    var json = {
    url:url,
    orderNo: orderNo,
    keep:keep,
...
getDeviceInfo:function(){
    var info = exec_syn("INFO","getDeviceInfo","null");
    ...
    /**
     * 返回原生页面
     * typeString( questionView 跳转到问题页 homeView 跳转主页面 )
     */
jumpView: function(typeString) {
    var data = {
        "viewKey": typeString
    }
    return exec_syn("VIEW", "jumpView", JSON.stringify(data));
},
    /**
     * 默注调用Token
     * phone {string} 电话号码
     ...
     */
phoneToken:function(phone, callback){
    var json = {
        "phone" : phone
    }
    exec_asyn("login", "phoneToken", JSON.stringify(json), callback, null);
}
}
...
showProgressDialog: function (title, msg) {
    ...
    BestpayHtml5.callNative(JSON.stringify(json), JSON.stringify(msgJson));
},
    //弹出等待框
...
callBackJs: function (result, key) {
    ...
    try{
        ...
        try{
            ...
            console.log('test message'+typeof message);
